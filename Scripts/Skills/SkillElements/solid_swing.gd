...
func _ready():
	...
	target_angle = sprite.get_rot() - target_angle
	sprite.set_rot(starting_angle)

	set_process(true)

func initialize(user,texture,swing_time,swing_start_time,swing_end_time,starting_angle,target_angle,hsize,vsize,damage,knockback,pullback):
	self.user = user
